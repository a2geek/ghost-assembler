...
public abstract class AsmAssert {
	...
	public static void assertEquals(byte[] expected, byte[] actual) {
		if ((expected == null && actual != null) || (expected != null && actual == null)) Assert.fail();
		if (expected.length != actual.length) {
			Assert.fail("Resulting lengths did not match!  " + expected.length + " <> " + actual.length);
		}
	...
	public static void assertEquals(long[] expected, long[] actual) {
		if ((expected == null && actual != null) || (expected != null && actual == null)) Assert.fail();
		if (expected.length != actual.length) Assert.fail("Resulting lengths did not match!");
		for (int i=0; i<expected.length; i++) {
			Assert.assertEquals(expected[i], actual[i]);
		}
	}
